...
export async function renderLlmsTxt(): Promise<string> {
  ...
  const lines = [
    ...
    "",
    "Cómo navegar: usa el buscador por municipio o recorre el listado por estado y tipo de sede. Cada ficha (`/sede/...`) incluye datos de contacto y mapa.",
    "",
    "## Navegación",
    "",
    mdLink("Inicio", "/", "Buscador y resumen del directorio"),
    mdLink("Buscar oficina", "/buscar/", "Filtro por palabra clave, estado o tipo (CABB, SARE, ORE)"),
